...
`SDD (Spec-Driven Development)` — это подход, в котором сначала фиксируется спецификация, а уже потом пишутся код, тесты и документация.

`Проще:` сначала определяем контракт системы, потом реализуем его.

`Как работает:`

1. Описываем поведение, входные данные, ограничения и ошибки
2. Получаем спецификацию как источник истины
3. Ревьюим и утверждаем спецификацию
4. По ней генерируем или пишем код и тесты

`Чем отличается:`

- От `vibe coding` — меньше угадывания и случайных решений
- От `TDD` — TDD стартует с тестов, SDD стартует со спецификации, из которой уже следуют тесты и код

`Простой пример:`

Для логина сначала фиксируем контракт:

- `POST /login`
- поля `user` и `pass`
- отдельные ошибки для пустых полей
- `200` для валидных данных

После этого AI или разработчик реализует фичу строго по этому описанию.

Источник:

- [IBM Technology: Spec-Driven Development](https://www.youtube.com/watch?v=mViFYTwWvcM)
